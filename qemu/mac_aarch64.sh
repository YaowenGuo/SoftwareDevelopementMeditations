#/usr/bin/env bash
QEMU=/opt/homebrew/Cellar/qemu/9.2.2/share/qemu
...
BOOT_DIR="-L $QEMU" # /Applications/UTM.app/Contents/Resources/qemu 
...
FRAMEWARE="-drive if=pflash,format=raw,unit=0,file.filename=$QEMU/edk2-aarch64-code.fd,file.locking=off,readonly=on"
# FRAMEWARE_VAR
FRAMEWARE_VAR="-drive if=pflash,format=raw,unit=1,file=$QEMU/edk2-arm-vars.fd"
...
# STARTUP_DISK="-hda $HOME/projects/linux/image/noble.img"
KEYBOARD="-device usb-kbd,bus=usb-bus.0"
MOUSE="-device usb-mouse,bus=usb-bus.0"
TOUCHPAD="-device usb-tablet,bus=usb-bus.0"
DISK="-device virtio-blk-pci,drive=drive2BC0901F-DA56-49C7-A0D2-98F71114CB44,bootindex=1 -drive if=none,media=disk,id=drive2BC0901F-DA56-49C7-A0D2-98F71114CB44,file=/Users/lim/Library/Containers/com.utmapp.UTM/Data/Documents/Linux.utm/Data/8B610956-0C53-4EE5-800D-0CD978273073.qcow2,discard=unmap,detect-zeroes=unmap"
# NET="-device virtio-net-pci,mac=3A:F0:6E:23:0C:A4,netdev=net0 -netdev vmnet-shared,id=net0"
OTHER="-device virtio-rng-pci -nodefaults -nographic -serial mon:stdio"
# Don’t create default devices.
# OTHER="-nodefaults -nographic"
