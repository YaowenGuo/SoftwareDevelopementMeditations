...
// 对应的 JNI 函数为，WebRTC 使用 脚本生成 JNI 调用，对应函数为 `JNI_Java` + 类名 + _ + `函数去掉 native 前缀`
// 例如 `nativeCreateAudioDeviceModule` 对应调用的函数为 `JNI_JavaAudioDeviceModule_CreateAudioDeviceModule`.
```
